...
- Put new runtime modules under the plugin-local `suwayomi/` namespace directory instead of adding more top-level `suwayomi_*.lua` files. Use slash-style Lua requires that work with KOReader's plugin loader package path, for example `require("suwayomi/api")` or `require("suwayomi/downloads/controller")`.
...
- Existing top-level `suwayomi_*.lua` modules are legacy migration targets. When touching one substantially, prefer moving or extracting toward the `suwayomi/` layout, with a thin compatibility wrapper only when needed for a small safe step.
...
- Release zips must include `_meta.lua`, `main.lua`, `README.md`, existing top-level runtime `suwayomi_*.lua` files while they remain, and the runtime `suwayomi/` directory once it exists. Specs, docs, CI files, and `AGENTS.md` are not runtime payload.
- For Android manual QA, push only runtime plugin files to `/sdcard/koreader/plugins/suwayomi_dl.koplugin/`; include `suwayomi/` when present and avoid pushing `.git`, `spec`, docs, or CI files.
