...
Library and chapter screens render usable saved information before nonmodal server requests. Complete successful server listings replace membership and order, including removals and genuine emptiness; an empty success suppresses reconstruction. A failed or incomplete response keeps the last usable list. Rejected cache writes preserve committed data and report failure, while the complete successful result may still control the live screen. Uncertain writes use the store fence. Ordinary Refresh reports failure; automatic failure stays quiet only when usable saved or reconstructed information remains.
