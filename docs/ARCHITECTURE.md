...
- `suwayomi/chapters/read_actions.lua`: read/unread actions and mark-previous orchestration. All explicit mark-read actions publish completion events after ledger persistence and immediate manual deletion. Independent snapshots capture a path only for a downloaded file that immediate deletion did not remove; successful immediate deletion preserves a pathless completion. Bulk events follow visible list order and wait for the entire ledger batch to be saved; server reconciliation does not publish completion events.
