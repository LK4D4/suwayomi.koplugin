...
Reconstruction reads only recorded existing paths in reader-return contexts and the chapter ledger, excluding known foreign endpoints. Unscoped legacy records may supply display information, never a new origin association: selection explains that server actions require an authoritative refresh. Known scoped records retain only their existing association. For known scoped numeric manga IDs, the standard relative thumbnail route reuses pre-cache covers. Reconstruction neither persists authoritative membership nor edits archives, progress, read state, download jobs, or deletion records. The next complete server result replaces it, including removals and emptiness.
