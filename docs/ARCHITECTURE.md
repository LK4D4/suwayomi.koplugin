...
Chapter-list and downloaded-ledger reconciliation use KOReader sidecar completion/progress evidence, never history membership. A history entry proves only that a file was opened; it cannot mark a chapter read or write completed metadata after an explicit unread choice has synchronized. Native completed-status-plus-close enrollment remains separate from this reconciliation.
