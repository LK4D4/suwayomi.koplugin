...
- `suwayomi/readsync/controller.lua`: pending read-sync scheduling, polling, retry, reconciliation, and checked completed-close enrollment before deferred service work. Automatic startup failures log and retry without a popup; explicit Sync reports startup failure. Successful startup is quiet.
