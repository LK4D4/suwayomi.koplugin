...
## UI, data, and packaging

Plugin-authored UI strings pass through [i18n](TRANSLATING.md); server values remain data. Diagnostics use opt-in redaction. Downloads shows persisted errors and inspection outcomes; background retry/cleanup remains quiet except actionable warnings. Server downloaded state never proves device-local availability. New archives use the source-scoped layout in [AGENTS.md](../AGENTS.md#code-and-data-safety); path construction sanitizes external names and recognizes supported older filenames. Settings, API parsing, worker results, and progress files validate input at their boundaries.
