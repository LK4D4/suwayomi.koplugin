...
Chapter loads and action preloads supersede one another on a host. Manga/context identity, endpoint, request tokens, cancellation, timeout, and host retirement guard publication. Chapter browsing keeps saved rows usable while a request runs; modal action preloads still cancel when dismissed. A cold Manga info Open next unread action uses the saved or reconstructed list, its filter, and pending read choices before considering a preload; saved empty or all-read lists do not fall back to a stale manga hint. Open cancels chapter network work before guarded local archive verification. Complete empty results replace old context/menu/selection. Failed reloads retain the previous complete view and do not execute the failed action.
