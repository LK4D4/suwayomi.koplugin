...
- `suwayomi/chapters/read_actions.lua`: read/unread actions and mark-previous orchestration. Explicit downloaded mark-read actions publish completion events after ledger persistence and immediate manual deletion. Bulk events follow visible list order and wait for the entire ledger batch to be saved; server reconciliation does not publish completion events.
