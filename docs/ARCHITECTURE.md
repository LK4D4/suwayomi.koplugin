...
| Manga actions | `suwayomi/manga/controller.lua` owns saved-first chapter loading, async refresh/action preloads, library membership, first-unread, and manga-level actions. `action_menu.lua` shares action definitions with chapter menus. |
...
[ADR-0006](adr/0006-unified-download-job-attempt-ownership.md) concentrates job and attempt transitions in the queue owner. External modules ask it about chapter ownership, exact stopping-attempt identity, and status; they do not access its pending/active/stopping maps. Refill still composes automatic admission with request consumption in one checked document transaction, through the queue's document-admission method.
...
- Completion commits queue state, ledger path, and reader-return context together, preserving current read/pending-sync choices. `service.lua` composes that transaction without mutating its attempt input and returns effective/pending archive generations; the queue owner alone applies those outcomes. A rejected completion save retains in-session ownership and retries bookkeeping, not the transfer. UI notification follows persistence and snapshot updates.
...
After a successful explicit nonzero policy selection, `downloads/controller.lua` offers KOReader's global `end_document_auto_mark` setting if disabled and the prompt is not retired. The saved buffer is independent of the response. `settings.lua` persists the plugin-wide `auto_mark_prompt_dismissed` flag after Enable or checked Keep disabled; only after that checked save succeeds does Enable change the native preference. KOReader owns persistence of its global setting. Native Back/outside dismissal ignores the checkbox and does not retire the prompt. No background trigger offers it; turning off Auto-download or changing the native preference later never overrides the user's choice. The compact native ConfirmBox and CheckButton live in `ui/downloads.lua`; download help points to KOReader's own control rather than duplicating it.
...
Reconstruction reads only recorded existing paths in reader-return contexts and the chapter ledger, excluding known foreign endpoints. Unscoped legacy records may supply local reading information, never a new origin association. Known scoped records retain only their existing association. For known scoped numeric manga IDs, the standard relative thumbnail route reuses pre-cache covers. Reconstruction neither persists authoritative membership nor edits archives, progress, read state, download jobs, or deletion records. The next complete server result replaces it, including removals and emptiness.

The client session captures endpoint, selection, widgets, and request identity; canceled, superseded, closed, or retired contexts cannot publish. Reopening Library retires the previous navigation branch. Ordinary Refresh retries; failures use KOReader's nonblocking notification. Category and manga rows stay open on selection so the native Menu selection callback does not retire the Library. A background category refresh never pushes a picker over a selected manga; membership acknowledgments update current rows by identity even after snapshot replacement. Category 0 includes manga with no explicit category membership. Raw and decoded thumbnails retain their existing keys; raw images decode locally and failed requests do not erase them. Undecodable cache entries use the placeholder and normal online fetch path without deleting the old file.

## Chapter loading and actions

`settings.lua:loadChapterCache` and `saveChapterCache` own versioned `chapter_cache` records keyed by manga ID under one normalized endpoint scope, using the same checked document store as Library. Entries contain independent manga metadata and complete chapter lists. Cached metadata fills absent fields only; incoming Library/source information wins. Complete empty lists are usable; rejected saves preserve committed data and report failure while the successful server membership still controls the live view.

`manga/controller.lua` renders a saved list before starting nonmodal chapter refresh. Missing lists reconstruct from recorded existing paths even beneath a usable Library cache; no matching records produce missing-information feedback, not authoritative emptiness. Successful complete responses replace order and membership. Display and action preloads share listing preparation: cache/reconstructed rendering and rejected cache writes remain observational, without acknowledging pending reads or updating sidecars, return contexts, or refill requests.

Recovered rows use existing IDs when known and recorded paths for local selection otherwise. Missing titles fall back to filenames. Unknown-scope, missing-ID, and foreign-ID-collision rows permit only Open/Verify; list selection and scanlator-filter recovery remain available. A current-scoped manga retains ordinary Refresh even when its recovered rows have no association. Their filter changes affect only the current view, leaving ID-keyed saved filters and refill policies untouched. Queue inspection's `read_only` option retains archive/freshness checks but does not adopt damage/completion state or notify mutation subscribers. Verification preserves existing origin associations; opening does not manufacture scope or overwrite an unassociated return context.

Successful listing replacement does not associate recovered archives. `readsync/ledger.lua` preserves unassociated archive records during remote read merging. Chapter-menu reconciliation requires a matching recorded endpoint, chapter identity, and path before writing native completion, updating the archive ledger, or enrolling native completion for read sync; a guessed readable path is insufficient. Known-associated archives retain normal read synchronization.

`chapters/local_downloads.lua` builds a transient recorded-path index shared by one synchronous menu render or list admission check. Return-context paths precede ledger paths, and known foreign-owned paths remain excluded. Callbacks do not retain this lookup; later actions resolve current records again.

Reader return checks the active reader, recorded path/context, endpoint, token, and live destination around native teardown, then delegates to normal chapter loading. It needs no network response before leaving the reader. KOReader remains the reading-progress owner; final-page progress alone never means completed.
...
Chapter loads and action preloads supersede one another on a host. Manga/context identity, endpoint, request tokens, cancellation, timeout, and host retirement guard publication. Chapter browsing keeps saved rows usable while a request runs; modal action preloads still cancel when dismissed. Both visibility and execution of a cold Manga info Open next unread action use the saved or reconstructed list, its filter, and pending read choices before falling back to the manga hint or a preload. Visibility does not install a chapter context or start network work; saved empty or all-read lists remain authoritative. Open cancels chapter network work before guarded local archive verification. Complete empty results replace old context/menu/selection. Failed reloads retain the previous complete view and do not execute the failed action.
...
- `ledger.lua` owns `commitChapterRead` for read-driven transactions and `saveChapterLedger` for reconciled snapshots. It enrolls refill against the previous read state, merges the ledger, and applies manual admission/revocation within one checked save. Committed admission outcomes return independent targets without mutating captures; read actions build completion snapshots from those results. Download completion and archive-removal bookkeeping retain their own transactions.
...
The transport regression spec retains real LuaSec option validation while replacing its downstream network. This catches HTTPS-only request-option failures that a stubbed `ssl.https` client would hide.
...
- `sandbox-inspector.lua` is installed only in the sandbox profile. It restricts KOReader's bundled inspector to loopback and a private token. Input observations expose metadata, not values; bounded authenticated edits use ordinary InputText methods. Visible controls include nested confirmation buttons. Its listener transfers between native UI hosts instead of rebinding while forked network workers retain the socket; closed hosts reject requests. It does not modify the production plugin or upstream runtime files.
...
| Saved-first chapters and offline return | `spec/complete_chapter_loading_spec.lua`, reader-return, chapter-action, download-service, and checked-settings specs cover observational recovery, retained pending choices, complete replacement, scope/collision guards, checked-write failures, and native handoff ownership. The [chapter-cache sandbox recipe](agents/testing.md#chapter-cache-acceptance) establishes real reading, resume, return, and restart behavior. |
...
| Read coordination, manual removal, retention | `spec/manual_read_completion_spec.lua`, `spec/suwayomi_readsync_ledger_spec.lua`, `spec/suwayomi_download_service_spec.lua`, and `spec/suwayomi_finished_cleanup_spec.lua` cover ordering, filesystem/persistence outcomes, ownership, and replacement safety. Ledger-interface controls cover independent committed targets and rejected/uncertain saves without leaked admission outcomes. |
