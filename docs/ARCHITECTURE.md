...
Startup runs once before admission. Unfinished queued/downloading jobs, including delayed retries, become quiet terminal failures with “Interrupted; retry download”, unless existing archive lookup permits completion bookkeeping. Permanent-failure diagnostics and unsupported keyed records remain unchanged. Startup does not sweep old temporary files. A failed startup save retries through the service while admission remains blocked.
...
This implements the reduced runtime scope of [ADR-0002](adr/0002-navigation-safe-download-ownership.md) and [spec #3](superpowers/specs/2026-09-06-navigation-safe-download-ownership.md). Device acceptance remains in #11. First use needs neither reboot nor storage relocation. An untracked child surviving an earlier process can still write shared paths and conflict with Retry; hot reload and simultaneous writable KOReader processes remain unsupported. No cross-process recovery or publication protocol is implemented. #4's checked settings store and reconciliation contract remain unchanged.
...
- `suwayomi/downloads/service.lua`: process singleton, disposable subscriptions, checked completion transaction, and quit integration.
...
- `suwayomi/downloads/queue.lua`: public KOReader-local queue facade, including startup interruption, busy-chapter checks, and persisted terminal failure count. In-session retry snapshots retain complete errors; startup interruption replaces unfinished-job diagnostics only. Queue transitions finish persistence and snapshot updates before notifying UI consumers. The home controller refreshes its existing Downloads button only when the terminal failure count changes; `main.lua` delivers service snapshots to the current chapter, Downloads, or home view. Duplicate enqueue messages route through `suwayomi/i18n.lua`; queue keys, persisted state, and progress state remain data.
...
- `suwayomi/chapters/manual_deletion.lua`: shared checked admission/revocation, archive-generation evidence, archive-only unlink/recovery, conditional retirement, quiet retries, and committed status snapshots; owned by the process service.
- `suwayomi/chapters/finished_cleanup.lua`: existing finish order, retention eligibility/positions, bounded traversal, retries, and affected-view refresh. New records retain immutable archive-target snapshots; unproved historical targets block rather than binding to later downloads. Pathless and retired-generation records keep their positions until displaced. Retention settings affect only this journal, not accepted manual requests. Ordinary Delete remains its destructive boundary.
- `suwayomi/chapters/read_actions.lua`: single, selected, previous, and unread coordination under ADR-0001's checked ordering. Capture precedes refresh; selected batches clear selection then reconcile visible non-target chapters into their shared ledger before the checked read/request commit. Singles commit and publish before refresh. Immutable completions publish in order before manual processing, sync, and keep-next-unread policy. Supplied ledgers and completion buffers do not bypass persistence. Failed or uncertain saves produce no durable-success claim or removal. One localized immediate summary separates read and archive outcomes.
...
- `spec/suwayomi_download_service_spec.lua` composes the shell, shared service, queue, checked settings store, real temporary files, deferred scheduling, public actions, and rendered views. It covers both navigation routes at concurrency two and one, zero-host completion and cleanup, detached/throwing subscribers, completion-save failures and reconciliation, known stopping workers, startup failure policy, explicit retry, and quit/relaunch. Older per-host queue and automatic-restart recovery assertions were replaced with these lifecycle contracts.
- `scripts/check-download-subprocess.lua` is an optional Linux probe against a supplied KOReader base checkout. It launches four real children with the unmodified helper, includes an already stopping child, and verifies bounded scheduler shutdown and confirmed exit. The implementation probe used base `9f9b664083d8cdab967638781096f5b089cb2e36` from KOReader `e0267724e9e8741837bbc71520bb0e36dd96a232`; all four stopped and shutdown added 0.000515 seconds on the test host. Device timing remains unverified under #11.
