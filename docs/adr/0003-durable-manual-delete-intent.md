...
Deletion after manual mark-read must not disappear because finish retention is disabled or its positions change. The [original investigation](../superpowers/audits/2026-09-06-issue-2-investigation.md#4-manual-mark-read-can-silently-abandon-a-failed-deletion) records that failure. Keep an accepted manual request independent of retention and bind it to one captured archive generation.
...
- Capture the exact target before refresh or metadata changes. Commit read-ledger changes and accepted manual requests through the checked shared store before archive removal. A failed or uncertain save starts no removal and makes no durable-success claim. KOReader metadata and server synchronization are outside that transaction.
