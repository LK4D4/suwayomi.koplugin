...
- Reuse revised #5's process service, current-session known-worker tracking, #4's checked shared store, and total two-second shutdown deadline. #36 owns the minimal archive identity/removal coordination; #5 does not provide a publication or inherited-lock protocol. Keep durable, quiet retries across navigation, zero subscribers, and restart, with bounded fair processing and no retry-count abandonment. Persist enough archive-removal progress to recover after unlink but before bookkeeping.
...
The 2026-09-07 ADR-0002 revision replaces prerequisites #9/#10 with #5. References below to its former worker locks, publication protocol, or legacy gate are historical, not capabilities supplied by #5. Manual-deletion requirements remain owned by #12/#36 and cannot expand the navigation implementation.
