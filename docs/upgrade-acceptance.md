...
| Stale controls | Initial automatic downloads must finish with matching associations, pages, and no remaining queue jobs. A real background response then invalidates an open ordinary action; rejection feedback appears without replay or ledger changes, then fresh Off succeeds. |
| Publication stages | Separately injected checked-write faults retain response membership, stage-specific cache/read-choice state, and files. A labeled three-to-two response subset distinguishes cache persistence from visible membership; background read sync is held during these cases to isolate publication acknowledgment. Directory-sync uncertainty records disk contents without claiming confirmed persistence. Labeled empty/stale response controls exercise existing publication guards. Missing cache and recorded-path preconditions retain actual bytes but force ordinary Open next unread through action preload. Verify exact document, pages, rebuilt cache, and no display preparation inside preload; later verification refresh is a separate action. |
