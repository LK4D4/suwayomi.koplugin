...
Follow [sandbox setup](sandbox.md) with Linux x86_64/WSLg, Python 3, LuaJIT, and `xdotool`. Use the pinned releases, a graphical session, an English profile, and a fresh private root. Native input targets only the window whose process owns this sandbox profile. Keep other reading sessions untouched.
