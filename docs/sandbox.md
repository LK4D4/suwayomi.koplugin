...
Use this only for opt-in live server or KOReader verification. Select the required evidence in [the testing guide](agents/testing.md). Feature scenarios live in [testing recipes](testing-recipes.md).
...
Wait for **`server ready`**. This requires authenticated API seeding of the Local source library and rejection of a separate unauthenticated request, not just process creation. The launcher's Basic, Simple, or UI Login session stays in memory and independent of KOReader. No cookie or token is shared with the plugin, so readiness does not prove plugin authentication. See [authentication](authentication.md) when testing its protocol. In the second terminal, start KOReader:
...
If a port is reported busy, inspect it with `ss -ltn` and check the sandbox's owned-service status. Stop only services owned by this sandbox; choose another fresh root and ports when another process owns the listener.
