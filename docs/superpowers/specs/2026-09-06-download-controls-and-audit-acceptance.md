...
| Delete after manual mark-read | New plugin single/selected/previous mark-read actions may accept independent durable archive-only removal. Owning queued/running/stopping/finalizing work makes deletion busy/not accepted: preserve it and request deletion again after it finishes. Accepted requests continue with this setting Off; unread or a later accepted deliberate download revokes remaining work under #12. Sidecars/backups/metadata remain and are not unfinished deletion. Use chapter status and immediate summaries; no dedicated manual Downloads controls. |
...
4. Cleanup: reuse manual deletion with retention off and three; independent retention three over A/B/C with restart between B and C; retention one while still live then after close; final-page-without-completed-status negative control. For manual deletion, include busy downloads with request-again behavior, transient retry, and replacement downloads; verify all sidecars/backups/metadata remain. Inspect archives, metadata, durable requests, ledger, and chapter summaries/status together. Test independent retention behavior separately.
...
The user authorized autonomous planning and publication, choosing simplicity without further questions. The public specification contains the required behavior and acceptance contract; the checked-in ADRs preserve decision history. Publication does not establish runtime implementation or device acceptance.
