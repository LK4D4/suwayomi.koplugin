...
| --- | --- | --- |
| §1: process-wide download ownership | [Spec #3](https://github.com/LK4D4/suwayomi.koplugin/issues/3), ADR-0002 | #4 complete; revised #5 implements and #11 verifies. #6–#10 superseded/not planned. |
...
Original ticket routing is retained for unrelated work; consult live progress before starting. R1 (#26) keeps #25 and now uses revised #5 for process service/quit and waits for the replacement implementation's checked manual admission and automatic-work fences. R3 (#28) keeps #27 and waits for complete bulk coordination. C2 (#30) keeps #28 and waits for actual reduced outcomes plus necessary retention/identity coordination. C3 (#31) keeps #29/#30 and waits for replacement device acceptance, which also waits for #11. Busy manual deletion never cancels downloads; accepted archive-only deletion remains durable. These are genuine gates, not requirements to close parent specs.
...
- [ ] Reuse existing one-shot helper result allocation, request tokens, and known-child completion checks; reject stale results and defer cleanup while the known child runs. Only the parent writes shared settings. Include the helper in revised #5's total two-second best-effort quit budget; no inherited-lock or durable launch protocol is supplied by that dependency.
...
- [ ] Persist transient retry count/deadline with five-second exponential backoff capped at five minutes and no count abandonment. Recover recorded refill requests once after process-service initialization; navigation/sleep never starts a second recovery or resets deadlines. This does not automatically retry chapter jobs marked interrupted/failed under revised #3.
...
- [ ] Preserve/reconcile requests at crashes around enrollment, existing-helper launch, result delivery, and atomic queue-admission/consumption. Never restore an old ledger/cache over uncertain replacement or clear a newer request revision.
- [ ] Use actual temporary files and the existing subprocess helper to check result lifetime, stale-result rejection, no child-side shared write, and cleanup after known-child completion. Do not import the inherited-lock/orphan-isolation matrix removed from #5; leave untracked old helper files alone.
- [ ] Share revised #5's one total two-second best-effort shutdown budget across known helper and chapter workers. Retain durable refill requests and leave unconfirmed files untouched. No required final transaction, future UI tick, or cross-process ownership proof.
