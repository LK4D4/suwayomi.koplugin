...
**Proposed repair:** persist manual-delete intent independently of finish-retention policy. Report pending/skipped outcomes. Define active-download handling explicitly and bind intent to the existing archive/job, so a later deliberate download is not deleted accidentally. Local entry points: [delete actions](../../../suwayomi/chapters/delete_actions.lua), [read actions](../../../suwayomi/chapters/read_actions.lua), [finished cleanup](../../../suwayomi/chapters/finished_cleanup.lua).
...
## Planning disposition, September 6
...
| §4: durable manual deletion | [#12](https://github.com/LK4D4/suwayomi.koplugin/issues/12), ADR-0003 and ADR-0001 amendment | #13–#20 |
| §5: batch cap, selection, and retention explanations | [#23](https://github.com/LK4D4/suwayomi.koplugin/issues/23) | #29–#30, preserving #19 retention policy |
...
The [complete publication map](../plans/2026-09-06-remaining-issue-2-tickets.md) records all eight new implementation tickets, 13 verified native blocking links, and every new specification story. Pagination can begin immediately; cap disclosure is independent of pagination but waits for checked admission (#4); refill waits for existing ownership/manual-delete foundations. Already repaired failures receive regression obligations rather than duplicate repair tickets. The original GitHub report and parent specifications remain unchanged.
