...
describe("process-owned download navigation", function()
    ...
    local function publishArchive(active, terminal_progress)
        local path = directory .. "/" .. active.chapter.id .. ".cbz"
        ...
        assert(writer:addFileFromMemory("1.png", "synthetic page"))
        assert(writer:close())
        assert(archive.stamp(path, active.attempt_id, 1))
        ...
        assert.are.equal("valid", result.state)
        if terminal_progress then
    ...
    it("retains terminal progress and completion ownership until the child exits", function()
        ...
        advance(0)
        local active = queue:getActiveJob("m1:c1")
        local path, bytes = publishArchive(active, true)
        owner:close()
        advance(0.5)
        assert.is_not_nil(read(active.progress_path))
        ...
        assert.are.equal(bytes, read(path))
        assert.is_nil(read(active.progress_path))
        assert.are.equal(1, #workers)
    end)

    it("records terminal publication canceled before the first progress poll", function()
        local plugin = host("files")
        local queue = plugin:getDownloadQueue()
        assert(queue:enqueue(manga, chapters[1], directory))
        advance(0)
    ...
    it("records an owned late publication without terminal progress", function()
        local plugin = host("files")
        local queue = plugin:getDownloadQueue()
        assert(queue:enqueue(manga, chapters[1], directory))
        advance(0)
        local active = queue:getActiveJob("m1:c1")
        assert(queue:cancelPending(manga, chapters[1]))
        local path, bytes = publishArchive(active, false)
        workers[active.pid].alive = false
        advance(1)
        assert.are.equal(path, settings:loadChapterLedger()["m1:c1"].path)
        assert.are.equal(path, settings:loadReaderReturnContexts()[path].path)
    ...
    it("does not associate a replacement with canceled terminal progress", function()
        local plugin = host("files")
        local queue = plugin:getDownloadQueue()
        assert(queue:enqueue(manga, chapters[1], directory))
        advance(0)
        local active = queue:getActiveJob("m1:c1")
        local path = publishArchive(active, true)
        advance(0.5)
        assert(queue:cancelPending(manga, chapters[1]))
        local archive = require("suwayomi/downloads/archive")
        assert(archive.stamp(path, assert(archive.newAttemptId()), 1))
    ...
    for _, failure in ipairs({ "write", "sync_dir" }) do
        it("retries canceled completion bookkeeping after " .. failure .. " failure", function()
            local plugin, owner = host("files")
            local queue = plugin:getDownloadQueue()
            assert(queue:enqueue(manga, chapters[1], directory))
            advance(0)
            ...
            settings.store.io[failure] = function() return nil, "injected completion failure" end
            workers[active.pid].alive = false
            advance(0.5)
            assert.are.equal(1, queue:getActiveCount())
            assert.is_not_nil(read(active.progress_path))
            ...
            assert.are.equal(path, settings:loadReaderReturnContexts()[path].path)
            assert.are.equal("downloaded", queue:getStatus(manga, chapters[1]).state)
            assert.are.equal(1, queue:getStatus(manga, chapters[1]).current)
            assert.are.equal(1, queue:getStatus(manga, chapters[1]).total)
            assert.are.same({}, settings:loadDownloadQueue())
            assert.are.equal(0, queue:getActiveCount())
            assert.is_nil(read(active.progress_path))
            assert.are.equal(1, #workers)
    ...
    it("opens recovered local archives without adopting a foreign same-ID ledger or queue job", function()
        ...
        local prepared = assert(plugin:prepareChapterMenuItems(local_manga, recovered))
        local rows = plugin:buildChapterMenuItems(local_manga, prepared)
    ...
    it("keeps explicit repair authority through cancellation and late publication", function()
        ...
        advance(0)
        local active = assert(queue:getActiveJob("m1:c1"))
        assert.are.equal(1, queue:cancelAll())
        local _, replacement = publishArchive(active, true)
        workers[active.pid].alive = false
        advance(0.5)
        owner:close()
        advance(300)
