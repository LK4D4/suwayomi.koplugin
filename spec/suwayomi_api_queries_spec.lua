...
describe("suwayomi/api/queries", function()
    ...
    it("builds a source filter schema query", function()
        ...
        assert.is_nil(payload.query:match("Long!"))
        assert.is_nil(payload.query:match("%$id:%s*ID!"))
        assert.is_nil(payload.query:match("%$id:%s*String!"))
        assert.truthy(payload.query:match('source%(id:%s*"2499283573021220255"%)'))
        ...
        assert.truthy(payload.query:match("selectDefault:%s*default"))
        assert.truthy(payload.query:match("textDefault:%s*default"))
        assert.truthy(payload.query:match("checkBoxDefault:%s*default"))
        assert.truthy(payload.query:match("triStateDefault:%s*default"))
        assert.truthy(payload.query:match("sortDefault:%s*default%s*{%s*index%s+ascending%s*}"))
        assert.is_nil(payload.query:match("%.%.%. on SortFilter%s*{%s*name%s+values%s+default"))
        local _, sort_default_count = payload.query:gsub("sortDefault:%s*default", "")
        assert.is_true(sort_default_count >= 3)
        assert.truthy(payload.query:match("%.%.%. on GroupFilter"))
        assert.truthy(payload.query:match("filters"))
        assert.is_nil(payload.variables)

        local string_id_payload = decode_request(queries._buildSourceFiltersQuery('local "source"'))
    ...
    it("builds library and category queries", function()
        ...
        assert.truthy(library.query:match("firstUnreadChapter"))
        assert.is_nil(library.query:match("latestFetchedChapter"))
    ...
    it("builds single manga metadata queries", function()
        local manga = decode_request(queries._buildMangaByIdQuery("17"))

        assert.truthy(manga.query:match("GET_MANGA_BY_ID"))
        assert.truthy(manga.query:match("inLibrary"))
        assert.truthy(manga.query:match("source%s*{"))
        assert.are.equal(17, manga.variables.filter.id.equalTo)
        assert.are.equal(1, manga.variables.first)

        local legacy = decode_request(queries._buildLegacyMangaByIdQuery("17"))
        assert.truthy(legacy.query:match("GET_MANGA_BY_ID"))
        assert.truthy(legacy.query:match("thumbnailUrl"))
        assert.is_nil(legacy.query:match("author"))
        assert.is_nil(legacy.query:match("artist"))
        assert.is_nil(legacy.query:match("description"))
